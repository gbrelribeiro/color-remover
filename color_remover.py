...
TEXT      = "#ffffff"
TEXT_DIM  = "#888888"
SUCCESS   = "#2563eb"   # azul para o botão salvar
WARNING   = "#ffd166"
...
def remove_color(image_path: str, hex_color: str, tolerance: int) -> tuple[Image.Image, int]:
    ...
    return Image.fromarray(result.astype(np.uint8), "RGBA"), pixels_removed
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text="Removedor de Cor", font=("Segoe UI", 20, "bold"),
    ...
    def _build_controls(self, parent):
        ...
        # ── cor hex ───────────────────────────────────────────────────────────
        sec2 = self._section(parent, "Cor a remover (HEX)")

        hex_row = tk.Frame(sec2, bg=CARD)
        hex_row.pack(fill="x")

        tk.Label(hex_row, text="#", font=("Consolas", 13, "bold"),
        ...
        self._hex_entry.bind("<KeyRelease>", self._update_swatch)

        self._swatch = tk.Label(hex_row, width=3, bg="#ffffff",
                                relief="flat", bd=0)
        self._swatch.pack(side="left", padx=6, ipady=10)

        self._btn(hex_row, "Capturar da imagem",
                  self._pick_color_from_image, "#2a2a2a", side="left", padx=(8, 0))

        # ── tolerância ────────────────────────────────────────────────────────
        sec3 = self._section(parent, "Tolerância de cor")

        tol_row = tk.Frame(sec3, bg=CARD)
        tol_row.pack(fill="x")
        ...
        self._tol_slider = ttk.Scale(
            tol_row, from_=0, to=120, variable=self._tolerance_var,
        ...
        # ── botão processar ───────────────────────────────────────────────────
        self._process_btn = self._btn(parent, "▶  Remover Cor",
    ...
    def _update_swatch(self, _=None):
        val = self._hex_var.get().lstrip("#")
        if len(val) == 6:
            try:
                int(val, 16)
                self._swatch.config(bg="#" + val)
            except ValueError:
                pass
    ...
    def _pick_color_from_image(self):
        ...
        if not path or not os.path.exists(path):
            ...
            return
        ColorPickerWindow(self, path, self._hex_var, self._swatch)
    ...
    def _process(self):
        ...
        try:
            ...
            messagebox.showerror("Erro", "Código HEX inválido. Use o formato RRGGBB.")
            return

        self._process_btn.config(state="disabled", text="⏳  Processando…")
        self._status.set("Processando…")
        threading.Thread(target=self._run_removal,
                         args=(path, hex_color, self._tolerance_var.get()),
                         daemon=True).start()

    def _run_removal(self, path, hex_color, tolerance):
        try:
            result, removed = remove_color(path, hex_color, tolerance)
            self._result_img = result
            photo = make_preview(result.copy())
            self._preview_result = photo
            self.after(0, lambda: self._show_result(photo, removed))
        except Exception as e:
            self.after(0, lambda: messagebox.showerror("Erro", str(e)))
            self.after(0, lambda: self._reset_btn())

    def _show_result(self, photo, removed):
        self._canvas_result.delete("all")
        self._canvas_result.create_image(160, 120, image=photo)
        self._save_btn.config(state="normal", bg=ACCENT2, fg="#ffffff")
        self._status.set(f"✔  Concluído — {removed:,} pixels removidos.")
        self._reset_btn()

    def _reset_btn(self):
        self._process_btn.config(state="normal", text="▶  Remover Cor")
    ...
    def _save(self):
        ...
        hex_c = self._hex_var.get().lstrip("#").upper()
        name  = f"{stem}_sem_{hex_c}.png"
...
class ColorPickerWindow(tk.Toplevel):
    def __init__(self, master, image_path, hex_var, swatch):
        ...
        tk.Label(self, text="Clique sobre a cor desejada",
                 font=("Segoe UI", 8), bg=BG, fg=TEXT_DIM).pack(pady=4)
